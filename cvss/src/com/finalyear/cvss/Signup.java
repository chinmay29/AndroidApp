...
Button btnRegister;
EditText inputName;
EditText inputEmail_id;
EditText inputPassword;
EditText inputMob_no;
EditText inputlock;
EditText inputque;
Button forget;
Button list;
...
return out; }
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        inputEmail_id = (EditText) findViewById(R.id.email_id);
        inputPassword = (EditText) findViewById(R.id.pwd);
        inputMob_no = (EditText) findViewById(R.id.mob_no);
        inputlock = (EditText) findViewById(R.id.lock);
        inputque = (EditText) findViewById(R.id.ans);
        btnRegister = (Button) findViewById(R.id.btnRegister);
         
         btnRegister.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
        String uname = inputName.getText().toString();
        String email_id = inputEmail_id.getText().toString();
        String lock = inputlock.getText().toString();
        String sDemoMesage = inputPassword.getText().toString();
        String mob_no = inputMob_no.getText().toString();
        String ans= inputque.getText().toString();
        String password;
        ...
        System.out.println("Demo Key (base64): "+ new String(encode(demoKeyBytes)));
        System.out.println("Demo IV  (base64): "+ new String(encode(demoIVBytes)));

        byte[] demo1EncryptedBytes=null;
        try {
        demo1EncryptedBytes = demo1encrypt(demoKeyBytes, 
         demoIVBytes, sPadding, demoMesageBytes);
        } catch (Exception e1) {
        ...
        boolean v=true;
        boolean echeck;
		echeck=checkemail(email_id);
		boolean mob;
		mob=PhoneNumberUtils.isGlobalPhoneNumber(mob_no);
		
        if(uname.equalsIgnoreCase("")
        		   || email_id.equalsIgnoreCase("")
        		   || lock.equalsIgnoreCase("")
        		   || sDemoMesage.equalsIgnoreCase("")
        		   || mob_no.equalsIgnoreCase("")
        		...
        		{
        			Toast.makeText(Signup.this, "Enter valid email_id.", 
           		         Toast.LENGTH_SHORT).show();
            	v=false;
        		}
        		else if(mob==false)
        		{
        			Toast.makeText(Signup.this, "Enter valid mobile number", 
              		         Toast.LENGTH_SHORT).show();
              	v=false;
        		}
        		if(v==true)
        		{
        url="http://mobilelock.freeiz.com/test/index.php?uname="+uname+"&email_id="+email_id+"&mob_no="+mob_no+"&pwd="+password+"&imei="+imei+"&lock="+lock+"&ans="+ans;
		...
		try {
		    ...
		    if(response != null) {
		        ...
		        //if(line.equals)
		        Toast.makeText(Signup.this, line, Toast.LENGTH_SHORT).show();
		    } else {
		        Toast.makeText(Signup.this, "Unable to complete your request", Toast.LENGTH_LONG).show();
		    }
		} catch (ClientProtocolException e) {
		    Toast.makeText(Signup.this, "Caught ClientProtocolException", Toast.LENGTH_SHORT).show();
		} catch (IOException e) {
		    Toast.makeText(Signup.this, "Caught IOException", Toast.LENGTH_SHORT).show();
		} catch (Exception e) {
		    Toast.makeText(Signup.this, "Caught Exception", Toast.LENGTH_SHORT).show();
		}
        		}
	...
	private String convertStreamToString(InputStream is) {
	    ...
	    try {
	        line=rd.readLine();
	        boolean  b = line.startsWith("Registration");
	        boolean a = line.startsWith("Not");
	        boolean c = line.startsWith("Please");
	        boolean e = line.startsWith("Username");
	        //boolean h = line.startsWith("1");
	        ...
	        Toast.makeText(Signup.this, "Stream Exception", Toast.LENGTH_SHORT).show();
	    }
